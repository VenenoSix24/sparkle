...
const UPDATE_MANIFEST_URLS: Record<AppUpdateChannel, string> = {
  stable: 'https://github.com/VenenoSix24/sparkle/releases/latest/download/latest.yml',
  rolling: 'https://github.com/VenenoSix24/sparkle/releases/download/rolling/latest.yml'
}
...
export async function downloadAndInstallUpdate(version: string, tag?: string): Promise<void> {
  ...
  const baseUrl = `https://github.com/VenenoSix24/sparkle/releases/download/${releaseTag}/`
  ...
  const apiUrl = `https://api.github.com/repos/VenenoSix24/sparkle/releases/tags/${releaseTag}`
