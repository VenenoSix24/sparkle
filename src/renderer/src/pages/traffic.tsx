    ...
    >
      <div className="flex flex-col gap-3 p-2">
        <div className="grid grid-cols-4 gap-2">
          {[
            { label: '会话数', value: totalStats.count.toString() },
            { label: '上传', value: calcTraffic(totalStats.upload) },
            { label: '下载', value: calcTraffic(totalStats.download) },
            { label: '总计', value: calcTraffic(totalStats.total) }
          ].map(({ label, value }) => (
            <div
              key={label}
